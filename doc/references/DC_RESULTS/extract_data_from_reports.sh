...
for file in `ls $reportdir/DLX_*ns_*_timing.rpt`
do
  powfile=`echo $file | sed -e "s/$reportdir\/\(DLX_.*ns.*\)_timing/$reportdir\/\1_power/g"`
  areafile=`echo $file | sed -e "s/$reportdir\/\(DLX_.*ns.*\)_timing/$reportdir\/\1_area/g"`
  ns=`echo $file | sed -e "s/$reportdir\\/DLX_\(.*\)ns_.*.*/\\1/g"`
  #uW=`echo $file | sed -e "s/$reportdir\\/DLX_.*ns_\(.*\).*/\\1/g"`
  if [ $VERBOSE -eq 1 ]
  then
