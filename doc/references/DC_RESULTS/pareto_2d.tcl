...
set LIBRARY WORK
set NETLIST_DIR TRY_ME5/results/
set REPORT_DIR TRY_ME5/reports/
...
set USE_MAX_CAP 1  ;#if 1, adds max fanout constraint for the high-fanout RESET signal - remember to set the value of MAX_FANOUT below
set MAX_CAP 10
...
if {$TRYHARD == 1} {append ADVANCED_OPTS "-map_effort high -area_effort high -power_effort high "}
...
for {set i $MIN_PERIOD} {$i <= $MAX_PERIOD} {incr i} {
 ...
 if {$USE_WLM == 1} {set_wire_load_model -name $WLM}	;#characterize wires as a non-ideal load;
  ...
  if {$USE_MAX_CAP == 1} {set_max_capacitance $MAX_CAP RESET}
  ...
  if {$USE_MAX_CAP == 1} {append filename "_MCAP"} ;#a maximum fanout constraint was used
  do_reports $filename
}
